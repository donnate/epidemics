...
dualpathFusedL1 <- function(y, D, D0, gamma, approx=FALSE, maxsteps=2000,
  ...
  if (is.null(object)) {
    ...
    #cl = components(gr)
    q = 1 #cl$no                            # Number of clusters (assuming the cluster is connected)
    i = rep(1, n) #cl$membership                    # Cluster membership
    x = numeric(n)
    ...
    # Now all groups with at least two elements ### That's not going to work in or cse
    # oi = order(i)
    # cs = cumsum(tab)
    # grps = which(tab>1)
    # for (j in grps) {
    ...
    #cs = cumsum(tab)
    #grps = which(tab>1)
    Lj = crossprod(Matrix(D,sparse=TRUE))
    x = as.numeric(solve(Lj,y))
  ...
  }else {# If iterating an already started path
  ...
  tryCatch({
    while (k<=maxsteps && lams[k-1]>=minlam) {
      ...
      # Otherwise, find the next hitting time
        ...
        if (any(I0)) {
            L = crossprod(Matrix(D1,sparse=TRUE))
            xa = as.numeric(solve(L,y))
            xb = as.numeric(solve(L,Ds))
            nzq = nzq-1
          }
          else {
            fa= mean(y)
            fb = mean(Ds)
            L = crossprod(Matrix(D1[,(1:nrow(D1))[-1]],sparse=TRUE))
            xa[-1] = as.numeric(solve(L,(y-mean(y))[-1]))
            xb[-1] = as.numeric(solve(L,(Ds-mean(Ds))[-1]))
          }
