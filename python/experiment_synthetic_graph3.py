            ...
            for p in p_range:
                for beta in [0.2, 0.5, 0.7, 0.9]:
                    for steps in step_range:
                        ...
                        res_ssnal[np.where(res_ssnal <min_clip_pred)[0]] = 0
                        current_p = res_ssnal
                        current_p_observed = scenario['epidemic']['y_observed']
                        ground_truth  = scenario['epidemic']['true_p']
                        n_nodes = scenario['W'].shape[0]
                        ...
                        results_df.to_csv('~/Downloads/corrected_preds' +   args.namefile + '_' + 'CV' + str(nb_folds) + '_res_' +  args.graph_type + '.csv', index=False)
