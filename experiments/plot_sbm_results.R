...
res  = data %>%
  group_by(lambda, beta_epid, gamma_epid,  dc_heterogeneity, steps, heterogeneity_rates, nb_init, p_norm) %>%
...
ggplot(res_all %>% filter(beta_epid == 0.9,
                          gamma_epid == 0.1,
                          p_norm == 1,
                          steps ==1,
                          nb_init == 1),
