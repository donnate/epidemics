...
file_list <- list.files(folder_path, pattern = "^res_987", full.names = TRUE)
...
res  = data %>%
  ...
  group_by(lambda, beta_epid, gamma_epid, 
           proba_er, steps, heterogeneity_rates, nb_init, p_norm, mode,
           diffuse, propagation) %>%
